...
class Luna < Hero
	def initialize
		...
		@skill3_d = "Luna calls upon the lunar goddess to bless her or allay hero, granting #{@skill3_damage} bonus Hp. Mana cost: #{@skill3_mana_cost}"
		@ultimate_d = "Luna deals #{@ultimate_damage} damage to all enemy units and binds them in complete darkness, making them miss their next turn. Mana cost: #{@ultimate_mana_cost}"
...
class Lich < Hero
	...
	def initialize
		...
		@skill3_d = ""
		@ultimate_d = "Lich invokes a deadly chain of frost that bounces between enemies 6 times dealing #{@ultimate_damage} damage each time. Mana cost: #{@ultimate_mana_cost} "
...
class Juggernaut < Hero
	...
	def skill3(ally_hero)
		...
		puts "#{@name} used #{@skill3_name} on #{ally_hero.name} and healed #{@skill3_damage} hit points, losing #{@skill3_mana_cost} mana."
		puts "Press 'Enter' to continue..."
		gets.chomp
	end
...
class Tidehunter < Hero
	...
	def skill2(enemy_player_pool)
		...
		enemy_player_pool.each do |hero|
			if hero.hp != "DEAD"
				...
				puts "#{hero.name} took #{@skill2_damage} damage."
			end
		end	
		puts "Press 'Enter' to continue..."
		gets.chomp
	...
	def ultimate(enemy_player_pool)
		@mana = @mana - @ultimate_mana_cost
		puts "#{@name} used #{@ultimate_name}. Mana left: #{@mana}"
		enemy_player_pool.each do |hero|
			if hero.hp != "DEAD"
				hero.hp -= @ultimate_damage
				puts "#{hero.name} took #{@ultimate_damage} damage."
